...
const blockUser = asyncHandler(async (req, res) => {
  ...
  db.query(checkSql, [student_id], (checkErr, checkResult) => {
    ...
    db.query(blockSql, [student_id], (blockErr) => {
      ...
      return res
        .status(StatusCode.OK)
        .json({ message: `تم حظر  ${userName} بنجاح`, student_id, userName });
    });
  });
});
...
async function checkMedicalExamStatus(medicEx_id) {
  return new Promise((resolve, reject) => {
    db.query(
      "SELECT status FROM medical_examinations WHERE medicEx_id = ? AND status = 'مقبول'",
      [medicEx_id],
      (error, results) => {
        if (error) {
          reject(error);
...
async function checkExHospExist(exHosp_id) {
  return new Promise((resolve, reject) => {
    db.query(
      "SELECT * FROM external_hospitals WHERE exHosp_id = ?",
      [exHosp_id],
      (error, results) => {
        if (error) {
          reject(error);
...
async function insertTransferRecord(
  ...
  return new Promise((resolve, reject) => {
    const transferQuery =
      "INSERT INTO transfers (student_id, transferReason, notes, medicEx_id, clinic_id, exHosp_id) VALUES (?, ?, ?, ?, ?, ?)";
    const values = [
      student_id,
      transferReason,
      notes,
...
const transfer = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const updateSql =
      "UPDATE medical_examinations SET transfered = 1 WHERE medicEx_id = ?";
